<template>
  <div class="login-container">
    <el-form class="card-box login-form" autoComplete="on" :model="loginForm"  ref="loginForm" label-position="left">
      <h1 class="title">后台管理系统登录</h1>

      <el-form-item prop="username" class="login_userinput">
        <span class="svg-container svg-container_login">
          <svg-icon icon-class="user" />
        </span>
        <el-input name="username" type="text" v-model="loginForm.loginName" maxlength="20" autoComplete="on" placeholder="登录帐号" />
      </el-form-item>
  ...
  methods: {
    ...
    handleLogin() {
      ...
      this.http.post(this).url(this.config.login).params({
        ...
        session.saveUser(response.data.data)
